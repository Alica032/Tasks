...
//
    ...
    context.activate();
    {
        ...
        // Если у вас есть интеловский драйвер для запуска на процессоре - попробуйте запустить
        // на нем и взгляние на лог,
        // передав printLog=true - скорее всего в логе будет строчка вроде
        // Kernel <mandelbrot> was successfully vectorized (8)
        // это означает что драйвер смог векторизовать вычисления с помощью интринсик,
        // и если множитель векторизации 8, то
        ...
        // обратите внимание что и произвдительность относительно референсной ЦПУ реализации
        ...
        unsigned int global_work_size_x = (width + workGroupSize - 1) / workGroupSize * workGroupSize;
        unsigned int global_work_size_y = (height + workGroupSize - 1) / workGroupSize * workGroupSize;
        timer t;

        for (int i = 0; i < benchmarkingIters; ++i){
            kernel.exec(gpu::WorkSize(workGroupSize,workGroupSize, global_work_size_x, global_work_size_y),
                    result, width, height, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                    sizeX, sizeY, iterationsLimit);
            t.nextLap();
        }

        result.readN(gpu_results.ptr(), n);
        ...
        image.savePNG("mandelbrotGpu.png");
    }
